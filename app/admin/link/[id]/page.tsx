...
export default function LinkDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadEvent() {
      ...
      const { data: eventData } = await supabase
        .from('events').select('*').eq('id', id).eq('host_id', user.id).single()
      if (!eventData) { router.push('/admin/dashboard'); return }

      let eventToUse = eventData
      if (eventData.status === 'live') {
        const nowTime = Date.now()
        let expired = false
        if (eventData.ends_at) { expired = new Date(eventData.ends_at).getTime() < nowTime }
        else if (eventData.starts_at) { expired = new Date(eventData.starts_at).getTime() + (eventData.duration_hours * 60 * 60 * 1000) < nowTime }
        else { expired = new Date(eventData.created_at).getTime() + (15 * 60 * 60 * 1000) < nowTime }
        if (expired) {
          const { data: updatedEvent } = await supabase.from('events').update({ status: 'ended' }).eq('id', eventData.id).select().single()
          if (updatedEvent) eventToUse = updatedEvent
        }
      }

      setEvent(eventToUse)
      const { data: usersData } = await supabase.from('event_users').select('*').eq('event_id', id).order('created_at', { ascending: false })
      setUsers(usersData || [])
      setIsLoading(false)
    }
    loadEvent()
  ...
  useEffect(() => {
    if (!event) return
    const supabase = createClient()
    const channel = supabase.channel(`link-event-users-${id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'event_users', filter: `event_id=eq.${id}` }, async () => {
        const { data } = await supabase.from('event_users').select('*').eq('event_id', id).order('created_at', { ascending: false })
        setUsers(data || [])
      })
      .subscribe()
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      const now = Date.now()
      let endTime = event.ends_at ? new Date(event.ends_at).getTime() : event.starts_at ? new Date(event.starts_at).getTime() + (event.duration_hours * 60 * 60 * 1000) : new Date(event.created_at).getTime() + (event.duration_hours * 60 * 60 * 1000)
      const remaining = endTime - now
      if (remaining <= 0) { setTimeRemaining('Event ended'); return }
  ...
  const joinUrl = event ? `${typeof window !== 'undefined' ? window.location.origin : ''}/join?code=${event.event_code}` : ''
  ...
  async function toggleVip(userItem: EventUser) {
    const supabase = createClient()
    const { error } = await supabase.from('event_users').update({ is_vip: !userItem.is_vip }).eq('id', userItem.id)
    if (!error) {
      setUsers(users.map(u => u.id === userItem.id ? { ...u, is_vip: !u.is_vip } : u))
      toast({ title: 'Success', description: `${userItem.username} ${!userItem.is_vip ? 'is now a VIP' : 'is no longer a VIP'}` })
    }
  }

  if (isLoading || !event) {
    return (<main className="min-h-dvh flex items-center justify-center"><Spinner className="w-8 h-8" /></main>)
  }
      ...
      <div className="max-w-md mx-auto space-y-6">
        ...
        <Card className="border-primary/30 bg-primary/5">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-foreground text-base"><LinkIcon className="h-4 w-4 text-primary" /> Share this link</CardTitle>
            <CardDescription className="text-xs">Send this link for people to join online — no QR scan needed</CardDescription>
          </CardHeader>
        ...
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="flex items-center gap-2 text-muted-foreground"><Users className="h-4 w-4" /> Participants</span>
        ...
        <div className="space-y-3">
          <Button variant="default" className="w-full" size="lg" onClick={async () => {
            ...
            const { data: { user } } = await supabase.auth.getUser()
            if (!user) { router.push('/admin'); return }
            let { data: hostUser } = await supabase.from('event_users').select('*').eq('event_id', event.id).eq('auth_user_id', user.id).single()
            if (!hostUser) {
              ...
              const { data: newHost, error } = await supabase.from('event_users').insert({
                event_id: event.id, username, vibe_key: generateVibeKey(), session_token: generateSessionToken(),
            ...
            <Eye className="mr-2 h-5 w-5" /> Peep &amp; Pass
          </Button>
          <Button variant="outline" className="w-full" size="lg" onClick={() => router.push(`/admin/event/${event.id}/host-setup`)}>
            <ExternalLink className="mr-2 h-5 w-5" /> Host Setup
          </Button>
          <Button variant="destructive" className="w-full" size="lg" disabled={isUpdating} onClick={async () => {
            if (!window.confirm('Delete this event now? This will end the event and log everyone out.')) return
            setIsUpdating(true)
            try { await fetch(`/api/admin/events/${event.id}/delete`, { method: 'POST' }); router.push('/admin/dashboard') }
            finally { setIsUpdating(false) }
          }}>
            <Trash2 className="mr-2 h-5 w-5" /> {isUpdating ? 'Deleting…' : 'Delete Event'}
          </Button>
        </div>
        ...
        {users.length > 0 && (
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => window.location.reload()}><RefreshCw className="h-4 w-4" /></Button>
            </div>
                  ...
                  <CardContent className="p-3 flex items-center gap-3">
                    <div className="relative w-12 h-12 rounded-full overflow-hidden ring-2 ring-primary/20 shadow-lg flex-shrink-0">
                      {userItem.selfie_url && !imageErrors[userItem.id] ? (
                        <Image src={getOptimizedImageUrl(userItem.selfie_url, 96)} alt={userItem.username} fill className="object-cover" sizes="48px" priority={false} quality={90} onError={() => handleImageError(userItem.id)} style={{ objectFit: 'cover' }} />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {userItem.is_vip && <Badge variant="default" className="text-xs flex items-center gap-1" style={{ background: 'rgba(37, 99, 235, 0.2)', color: '#60a5fa', borderColor: 'rgba(37, 99, 235, 0.3)' }}><Crown className="h-3 w-3" /> VIP</Badge>}
                        {userItem.is_upgraded && !userItem.is_vip && <Badge variant="secondary" className="text-xs bg-blue-500/20 text-blue-400 border-blue-500/30">Upgraded</Badge>}
                      </div>
                      <p className="text-xs text-muted-foreground font-mono">{userItem.vibe_key}</p>
                    </div>
                    <Button variant="outline" size="sm" onClick={() => toggleVip(userItem)} className={userItem.is_vip ? 'bg-blue-500/10 border-blue-500/30 text-blue-400 hover:bg-blue-500/20' : ''}>
